...
Íconos de camas dibujados con tkinter Canvas.
"""
...
TIPO_ACCENT  = {"2": "#34d399", "3": "#fbbf24", "4": "#f472b6"}

# Configuraciones de posición de camas (x, y, w, h) para cada tipo
_BED_CONFIGS = {
    "2": [(4,  16, 16, 10), (24, 16, 16, 10)],
    "3": [(2,  16, 12, 10), (16, 16, 12, 10), (30, 16, 12, 10)],
    "4": [(2,   8, 12,  9), (18,  8, 12,  9),
          (2,  22, 12,  9), (18, 22, 12,  9)],
}


def _draw_beds_canvas(parent, tipo, bg_color, fg_color, accent_color, size=44):
    """
    Dibuja N camas en un tkinter Canvas según el tipo de habitación.
    Retorna el widget Canvas listo para colocar con place/pack/grid.
    """
    canvas = tk.Canvas(
        parent, width=size, height=size,
        bg=bg_color, highlightthickness=0
    )
    configs = _BED_CONFIGS.get(str(tipo), [])
    for (x, y, w, h) in configs:
        # Pata izquierda
...
class HabitacionesScreen(ctk.CTkFrame):
    ...
    def _build_stats(self):
        ...
        self._stat_total = self._stat_card(sf, 0, "Total",      "0", None,  CLR_SKY_DARK, "#dbeafe")
        self._stat_dob   = self._stat_card(sf, 1, "Dobles",     "0", "2",   "#065f46",    "#d1fae5")
        self._stat_tri   = self._stat_card(sf, 2, "Triples",    "0", "3",   "#92400e",    "#fef3c7")
        self._stat_cua   = self._stat_card(sf, 3, "Cuádruples", "0", "4",   "#9d174d",    "#fce7f3")

    def _stat_card(self, parent, col, title, value, bed_tipo, ic_color, ib_color):
        ...
        # Cuadro del ícono
        ib_f = ctk.CTkFrame(inner, fg_color=ib_color, corner_radius=10, width=44, height=44)
        ib_f.pack(side="left")
        ib_f.pack_propagate(False)

        if bed_tipo is not None:
            # Ícono de camas con Canvas
            accent = TIPO_ACCENT.get(bed_tipo, "#94a3b8")
            bed_w = _draw_beds_canvas(ib_f, bed_tipo,
                                      bg_color=ib_color,
                                      fg_color=ic_color,
                                      accent_color=accent,
                                      size=44)
            bed_w.place(relx=0.5, rely=0.5, anchor="center")
        else:
            # Total: ícono genérico de texto
            ctk.CTkLabel(ib_f, text="▣",
                         font=ctk.CTkFont(size=20),
                         text_color=ic_color).place(relx=0.5, rely=0.5, anchor="center")

        tc = ctk.CTkFrame(inner, fg_color="transparent")
        tc.pack(side="left", padx=(12, 0))
        lbl = ctk.CTkLabel(tc, text=value,
                           font=ctk.CTkFont(size=26, weight="bold"),
                           text_color=CLR_TEXT)
        lbl.pack(anchor="w")
        ctk.CTkLabel(tc, text=title,
                     font=ctk.CTkFont(size=11),
    ...
    def _render_grid(self):
        ...
        for idx, r in enumerate(rows):
            ...
            t_accent = TIPO_ACCENT.get(tipo, "#94a3b8")

            row_idx = idx // COLS
            col_idx = idx % COLS

            card = ctk.CTkFrame(self._grid_scroll, fg_color=CLR_WHITE,
                                corner_radius=14, border_width=2,
                                border_color=CLR_BORDER)
            card.grid(row=row_idx, column=col_idx, padx=8, pady=8, sticky="ew")

            # ── Banda superior con ícono de camas ──
            top_band = ctk.CTkFrame(card, fg_color=t_color, corner_radius=0, height=64)
            top_band.pack(fill="x")
            top_band.pack_propagate(False)

            bed_canvas = _draw_beds_canvas(
                top_band, tipo,
                bg_color=t_color,
                fg_color=t_fg,
                accent_color=t_accent,
                size=52
            )
            bed_canvas.place(relx=0.5, rely=0.5, anchor="center")

            # ── Número de habitación ──
            ctk.CTkLabel(card, text=f"Hab. #{numero}",
                         font=ctk.CTkFont(size=14, weight="bold"),
                         text_color=CLR_TEXT).pack(pady=(10, 2))
            ...
            # ── Selección al hacer clic en cualquier parte de la tarjeta ──
            def _sel(h=hid, c2=card):
                self._select(h, c2)

            for widget in [card, top_band, bed_canvas]:
    ...
    def _select(self, hid, card):
        ...
        self._selected_frame = card
        self._set_actions(True)
        self._lbl_sel.configure(
            text=f"Habitación ID {hid} seleccionada",
    ...
    def _open_form(self, datos=None):
        ...
        ctk.CTkLabel(hdr,
                     text="Nueva habitación" if not edit else "Editar habitación",
                     font=ctk.CTkFont(size=16, weight="bold"),
                     text_color=CLR_TEXT).pack(side="left", padx=24, pady=14)
        ...
        for val, nombre in [("2", "Doble"), ("3", "Triple"), ("4", "Cuádruple")]:
            f = ctk.CTkFrame(tipo_frame, fg_color="transparent")
            f.pack(side="left", expand=True, fill="x", padx=3)
            ctk.CTkRadioButton(f, text=nombre, variable=tipo_var, value=val,
                               fg_color=CLR_SKY_DARK, hover_color=CLR_SKY_XDARK,
                               text_color=CLR_TEXT_SOFT,
    ...
    def _confirm_delete(self):
        ...
        ctk.CTkLabel(dialog, text="Los residentes asignados quedarán sin habitación.",
                     font=ctk.CTkFont(size=11),
                     text_color=CLR_MUTED).pack(pady=(4, 0))
